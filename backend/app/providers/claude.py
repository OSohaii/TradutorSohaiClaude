"""Claude (Anthropic) provider: OCR (vision) + translation via Messages API.

Uses Claude Sonnet and Claude Haiku models. The OCR function uses the vision
capability to read manga pages; the translation function uses regular
text messages. Follows the same retry-with-backoff pattern as openai.py.
"""
...
_RETRY_SUBSTRINGS = ("429", "rate_limit", "quota", "Too Many Requests", "overloaded")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if is_rate_limit and attempt < max_retries - 1:
                ...
                logger.warning("Claude rate-limit, retrying in %.1fs", delay)
...
def _classify_error(exc: Exception) -> ProviderError:
    msg = str(exc)
    status = getattr(exc, "status_code", None) or getattr(exc, "status", None)
    if status in (401, 403) or "invalid_api_key" in msg or "invalid x-api-key" in msg.lower():
        return ProviderError(
            ErrorCode.AUTH,
            "claude",
            "Chave Claude/Anthropic invalida ou sem acesso ao modelo solicitado.",
        )
    if status == 429 or any(s in msg for s in _RETRY_SUBSTRINGS):
        return ProviderError(
            ErrorCode.RATE_LIMIT,
            "claude",
            "Limite de uso do Claude atingido. Tente novamente em alguns instantes.",
            recoverable=True,
        )
    return ProviderError(ErrorCode.UNKNOWN, "claude", f"Falha Claude: {msg}")
...
Your output must be strict JSON following the schema provided. Do not include any text outside the JSON object."""
...
async def process_manga_page(
    ...
    *,
    ...
    """Run a single OCR (or OCR+translate) pass over an image using Claude vision."""
    image_b64 = base64.b64encode(image_bytes).decode("ascii")

    payload = {
        "model": model,
        "max_tokens": 4096,
        "system": _system_instruction(source_language),
        "messages": [
            {
                "role": "user",
                "content": [
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        async def _call():
            ...
            if resp.status_code != 200:
                ...
                exc = Exception(f"Claude API error {resp.status_code}: {error_body}")
    ...
    if not text:
        raise ProviderError(ErrorCode.UNKNOWN, "claude", "Resposta vazia do Claude.")

    try:
        parsed = json.loads(text)
    except json.JSONDecodeError as exc:
        raise ProviderError(
            ErrorCode.UNKNOWN, "claude", f"JSON invalido do Claude: {exc}"
        ) from exc
...
async def translate_bubbles(
    ...
    *,
    ...
    """Translate already-extracted bubbles using Claude Messages API."""
    if not bubbles:
...
Respond with JSON only: {{"translations": ["translated line 1", "translated line 2", ...]}}"""

    payload = {
        "model": model,
        "max_tokens": 4096,
        "system": "You are a professional manga translator. Follow the Fantasy/RPG terminology rules strictly. Respond only with valid JSON.",
        "messages": [
    ...
    if not text:
        raise ProviderError(
            ErrorCode.UNKNOWN, "claude", "Resposta vazia da traducao Claude."
        )
    ...
    try:
        ...
        raise ProviderError(
            ErrorCode.UNKNOWN, "claude", f"JSON invalido na traducao: {exc}"
        ) from exc

    translations = parsed.get("translations") or []
    if len(translations) != len(bubbles):
        logger.warning(
            "Claude translation length mismatch (got %d, expected %d); keeping originals",
            len(translations),
            len(bubbles),
        )
