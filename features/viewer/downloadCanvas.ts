...
export interface DownloadCanvasParams extends RenderCanvasParams {
  imgElement: HTMLImageElement;
  canvasElement: HTMLCanvasElement | null;
}

/**
 * Loads an image from a URL (base64 or http) into an HTMLImageElement.
 */
...
/**
 * Core rendering logic: renders a ProcessedImage onto a canvas with all
 * overlays (mask, bubbles) applied. Returns the canvas element.
 * This function loads images via new Image() so it does not require
 * existing DOM elements.
 */
export async function renderToCanvas(params: RenderCanvasParams): Promise<HTMLCanvasElement> {
  const {
    image,
    defaultFont,
    globalBold,
  ...
  // Load the base image
  const imgSrc = image.base64 || image.imageUrl;
  ...
  if (image.maskDataUrl) {
    try {
      ...
      // mask may not be loadable (e.g. revoked blob), skip
    }
  }
